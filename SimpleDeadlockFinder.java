...
public class SimpleDeadlockFinder {
			...
			for (int y = 0; y < board.height; ++y) {
				if (board.board[x][y] == '.') {
					...
					while (!q.isEmpty()) {
						...
						for (BoardPos newBoardPos : findPossiblePulls(board,
								boardPos)) {
	...
	/*
	 * Help method for method notDeadLockSquare, objective is to find possible
	 * pulls from a boardPos and return these in a vector of characters.
	 */
	public static Vector<BoardPos> findPossiblePulls(Board board,
		...
		if (board.currY - 2 > 0) {
			// UP
			if (board.board[board.currX][board.currY - 2] == ' '
					|| board.board[board.currX][board.currY - 2] == '.')
		...
		if (board.currX - 2 > 0) {
			// LEFT
			if (board.board[board.currX - 2][board.currY] == ' '
					|| board.board[board.currX - 2][board.currY] == '.')
	...
	/*
	 * Help method for notDeadLockSquare, objective is to perform a pull(u, d,
	 * l, r) based on current positions on the board. This way notDeadLockSquare
	 * gets a new board with different positions.
	 */
